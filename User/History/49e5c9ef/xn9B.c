...
SYSCALL_DEFINE3(sse_formula, int, a, int, b, int, c)
{
    ...
    if ( a == 0 ) {
        ...
        integer = quotient / -10000000;
        fraction = quotient % 10000000;

        if ( integer == 0 && fraction != 0 ) printk("SSE3044: %dx^2 + %dx + %d = 0 -> can be -%d.%d\n", a, b, c, integer, fraction);
        else printk("SSE3044: %dx^2 + %dx + %d = 0 -> can be %d.%d\n", a, b, c, integer, fraction);
